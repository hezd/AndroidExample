 ...
 * @description
 */
object CustomDnsNetworkRegister {
    ...
    fun register(application: Application) {
        ...
        AndroidResolverConfigProvider.setContext(application)


        // 2. 注册网络变化回调，以便在网络变化时刷新 DNS 配置
        // 当网络类型切换（例如Wi-Fi到移动数据）或IP地址/DNS服务器信息更新时，
        // 刷新 ResolverConfig 能够确保 dnsjava 使用最新的系统DNS服务器列表。
        registerNetworkChangeCallback(application)
    }

    @JvmStatic
    fun registerNetworkChangeCallback(application: Application) {
